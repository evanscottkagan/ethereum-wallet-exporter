...
func TestPriceQuerierFail(t *testing.T) {
	...
	config := &configPkg.Config{Chains: []configPkg.Chain{{
		Name:   "chain",
		Denoms: []configPkg.DenomInfo{{Denom: "eth", CoingeckoCurrency: "ethereum"}},
	}}}
...
func TestPriceQuerierOk(t *testing.T) {
	...
	httpmock.RegisterResponder(
		"GET",
		"https://api.coingecko.com/api/v3/simple/price?ids=ethereum,random&vs_currencies=usd",
	...
	config := &configPkg.Config{Chains: []configPkg.Chain{{
		...
		Denoms: []configPkg.DenomInfo{
			{Denom: "eth", CoingeckoCurrency: "ethereum"},
